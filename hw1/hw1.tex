...
\section{Preface:}
Throughout this homework I use contravariant composition (so \((f;g)(x) = (fg)(x) = g(f(x))\)) on morphisms. Composition (and use) of functors and natural transformations will stay as usual.
...
\begin{homeworkProblem}
    ...
    Thus, \(I\) is indeed a functor. Let now \(f\) and \(g\) both be from \(\C{X,Y}\), such that \(I(f) = I(g)\). Then we have \(φ : \p{\id_X,f} ≡ \p{\id_X, g}\). From the equality \(φ\id_X = \id_X\) it follows that \(φ = \id_X\). Then it follows from \(φf = g\) that \(f = g\) so \(I\) is faithful.
...
\begin{homeworkProblem}
    ...
    \noindent
    \textbf{Lemma:} The maps \(η_X\) are monic.
    \begin{proof}[Proof of lemma]
        ...
        Then by pullback laws there exists a unique \(φ : X → X\) such that \(φ;\id_X = \id_X\) and \(φ;f = g\). From the first equality we get \(φ = \id_X\) which means that \(f = g\) follows and thus, \(η_Y\) is a mono.
    \end{proof}
    \begin{proof}
...
\begin{homeworkProblem}
    ...
    \begin{proof}
        ...
        By the pullback lemma we again conclude that the left square is a pullback.
...
\begin{homeworkProblem}
    ...
    \begin{proof}[\((⇒)\)]
        ...
        Take arbitrary \(\b{m,g} ∈ \Cpar{X',Y}\) and \(f ∈ \C{X,X'}\). Then we have \(\p{Ff;α_X}\b{m,g} = t_{\bar{m},\bar{f};g}\) and \(\p{α_{X'};f^*}\b{m,g} = f;t_{m,g}\).
    ...
    \begin{proof}[\((⇐)\)]
        ...
        For the rest of the proof keep the following diagram in mind (note that we do not assert the commutativity or existence of any of the maps indicated by convention):
        \[\begin{tikzcd}
            {P}\\
            {} & {X₀} & {Y}\\
            {} & {X}  & {\tY}
        ...
        The pullback square above gives us the equality \(\bar{q}m = \bar{m}q\).
        Putting it all together we get the equalities \(φf = \bar{m}^{-1}\bar{q}f = p\) and \(φm = \bar{m}^{-1}\bar{q}m = \bar{m}^{-1}\bar{m}q = q\), making the square a pullback.
